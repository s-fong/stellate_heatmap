...
class MeshType_1d_path1(Scaffold_base):
    ...
    def generateMesh(region, options):
        ...
        pressure = findOrCreateFieldFiniteElement(fm, name='pressure', components_count=1, type_coordinate=False)
        ...
        pressureList = read_data_to_list('C:\\Users\\mlin865\\Curated Data Blackfynn\\Dinning_Human_Motility\\primary\\'
                                         '27_F_1943\\27_F_1943\\27_F_1943.txt')[2]
        ...
        for n in range(elementsCount + 1):
            ...
            cache.setNode(node)
            pressure.setNodeParameters(cache, -1, Node.VALUE_LABEL_VALUE, 1, pressureList[1000][n])
        ...
        pressureElementtemplate.setElementShapeType(Element.SHAPE_TYPE_LINE)
        linearLagrangeBasis = fm.createElementbasis(1, Elementbasis.FUNCTION_TYPE_LINEAR_LAGRANGE)
        eft1 = mesh.createElementfieldtemplate(linearLagrangeBasis)
        pressureElementtemplate.defineField(pressure, -1, eft1)
...
def read_data_to_list(filename):
    """
    Read txt files to extract pressure datapoints into lists.
    :param filename: filename of txt file
    :return a list of timepoints, event points and datapoints.
    """
    timeList = []
    eventList = []
    dataList = []
    ...
    for i, line in enumerate(open(filename)):
        ...
        eventList.append(float(line.split('\t')[1]))
        data = []
        numberOfColumns = len(line.split('\t'))
        for j in range(2, numberOfColumns):
            datapoint = line.split('\t')[j]
    ...
    return timeList, eventList, dataList, counter # counter just for checks
