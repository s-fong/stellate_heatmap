...
def parse_tree_with_anatomical_terms(tree_root):
    if 'contour' in tree_root.attrib:
        pass
    elif tree_root.get("type") == 'Axon':
            ...
            if property_set.get("name") == "Set":
                for child in property_set.iter():
                    if "}" in child.tag:
                        tag = child.tag.split('}')
                        if "s" in tag:
                            anatomical_term_new = "".join(child.itertext())
                            if anatomical_term == None:
                                anatomical_term = anatomical_term_new
                            else:
                                if 'innervates' in anatomical_term_new:
                                    dendrite = anatomical_term
                                    anatomical_term = anatomical_term_new
                                    return anatomical_term, dendrite
                                elif 'innervates' in anatomical_term:
                                    dendrite = anatomical_term_new
                                    return anatomical_term, dendrite
        return anatomical_term, dendrite
    else:
        for property_set in tree_root.iter():
...
def parse_tree(tree_root):
    anatomical_term, dendrite = parse_tree_with_anatomical_terms(tree_root)
    tree = {'colour': tree_root.attrib['color'], 'rgb': convert_hex_to_rgb(tree_root.attrib['color']),
        ...
        'anatomical term': anatomical_term,
...
def parse_contour(contour_root):
    ...
    for child in contour_root:
        ...
        if raw_tag == "point":
            ...
            # get soma and assoc dendrite branch
            if 'Soma' in contour['name']:
                if child.get("name") == "Set":
                    for ch in child.iter():
                        if "}" in ch.tag:
                            tag = ch.tag.split('}')
                            if "s" in tag:
                                dendrite = "".join(ch.itertext())
                                if 'all' not in dendrite.lower():
                                    #######################################################################################
                                    if False: # True for all normal purposes EXCEPT in finding soma-nerve distances of FRAGMENTS
                                    #######################################################################################
                                        contour["name"] = contour["name"] + ": " + dendrite
                                    else:
                                        contour['Dendrite ID'] = dendrite
...
def main(input_file):
    options = {}
    if input_file:
        options["external_annotation"] = None
        contents = read_xml(input_file)
        write_ex(input_file + '.ex', contents, options)
...
if __name__ == "__main__":
    file = 'C:\\Users\\sfon036\\Google Drive\\SPARC_work\\stellate\\stellate_xml_scaffold\\xml_ex_files\\03jan18_10X_40X_18103000-049 (c1)_18103090-135 (c6)_working.xml'
    main(input_file=file)
